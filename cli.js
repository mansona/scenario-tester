...
yargs(process.argv.slice(2))
    ...
    .command('list', 'List all the scenarios defined in your test suite', yargs => yargs
    .option('files', {
    type: 'string',
    ...
    .array('require')
    .option('matrix', { type: 'string' }), async (argv) => {
    let mod = await import('./list.js');
    await mod.printList(argv);
})
    .command('output', 'Write out one of your test scenario apps as a real app on disk, so you can inspect, debug, and run it', yargs => yargs
    .option('scenario', {
    type: 'string',
